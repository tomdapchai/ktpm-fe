...
export async function GET(request: NextRequest, { params }: { params: { specialization: string } }) {
  ...
  const filteredStaff = staffData.filter((staff) =>
    staff.specializations.some((spec) => spec.toLowerCase().includes(specialization.toLowerCase())),
  )

  return NextResponse.json({
    status: 1073741824,
    message: `Staff with specialization ${specialization} retrieved successfully`,
    data: filteredStaff,
  })
}
